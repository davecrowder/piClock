...
fn main() {
    ...
    spawn(move || {
        for (command, value) in display_rx {
            ...
            display.write(&buffer).expect("Failed to write to SPI.");
        }
    });
    ...
    switch1
        .set_async_interrupt(Trigger::FallingEdge, move |level| {
            main_tx_switch1
                .send(MainMessage::ButtonChange(1, level))
                .unwrap();
        })
        .unwrap();
    ...
    switch2
        .set_async_interrupt(Trigger::FallingEdge, move |level| {
            main_tx_switch2
                .send(MainMessage::ButtonChange(2, level))
                .unwrap();
        })
        .unwrap();

    // Create "pendulum" process and set it to generate a message every second,
    // on the second.
    spawn(move || loop {
        let dt = Local::now();
...
fn disp_time(display_tx: &std::sync::mpsc::Sender<(u8, u8)>, display_inverted: bool) {
    ...
    if display_inverted {
        display_tx
            .send((
                0x8,
                decode_digit(second_low, DigitOrientation::Inverted, false),
            ))
            .unwrap();
        display_tx
            .send((
                0x7,
                decode_digit(second_high, DigitOrientation::Inverted, false),
            ))
            .unwrap();
        ...
        display_tx
            .send((
                0x5,
                decode_digit(minute_low, DigitOrientation::Inverted, false),
            ))
            .unwrap();
        display_tx
            .send((
                0x4,
                decode_digit(minute_high, DigitOrientation::Inverted, false),
            ))
            .unwrap();
        ...
        display_tx
            .send((
                0x2,
                decode_digit(hour_low, DigitOrientation::Inverted, false),
            ))
            .unwrap();
        display_tx
            .send((
                0x1,
                decode_digit(hour_high, DigitOrientation::Inverted, false),
            ))
            .unwrap();
    } else {
        display_tx
            .send((
                0x1,
                decode_digit(second_low, DigitOrientation::Normal, false),
            ))
            .unwrap();
        display_tx
            .send((
                0x2,
                decode_digit(second_high, DigitOrientation::Normal, false),
            ))
            .unwrap();
        display_tx.send((0x3, 0x1)).unwrap();
        display_tx
            .send((
                0x4,
                decode_digit(minute_low, DigitOrientation::Normal, false),
            ))
            .unwrap();
        display_tx
            .send((
                0x5,
                decode_digit(minute_high, DigitOrientation::Normal, false),
            ))
            .unwrap();
        display_tx.send((0x6, 0x1)).unwrap();
        display_tx
            .send((0x7, decode_digit(hour_low, DigitOrientation::Normal, false)))
            .unwrap();
        display_tx
            .send((
                0x8,
                decode_digit(hour_high, DigitOrientation::Normal, false),
            ))
            .unwrap();
    }
}
...
fn decode_digit(digit: u8, orientation: DigitOrientation, dp: bool) -> u8 {
    ...
    match orientation {
        DigitOrientation::Normal => match digit {
            0 => {
                return_value += 126;
            }
            1 => {
        ...
        DigitOrientation::Inverted => match digit {
            0 => {
                return_value += 126;
            }
            1 => {
